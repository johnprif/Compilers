...
def checkComments(varComment):
	if(varComment=="/*"):
		while(1):
			tempComment=lex()
			if(tempComment==EOFError || tempComment=="*/"):
				break
			elif(tempComment=="/*" || tempComment=="//"):
				print("Error in comment at line=", l)
				sys.exit()
	elif(varComment=="//"):
		while(1):
			tempComment=lex()
			if(tempComment==EOFError || tempComment=='\n'):
				break
			elif(tempComment=="//" || tempComment=="/*" || tempComment=="*/"):
				print("Error in comment at line=", l)
				sys.exit()
...
def varlist():
	...
	if(token not in reservedWords):
		...
		if(token==','):
			...
			print("Error at declaration variable at line=",l)
			sys.exit()
	else:
		print("Incompatible varable name at line=",l)
		sys.exit()
	return 1
...
def subprogram():	
	...
	if(token=="function"):
		...
		if(functionID in reservedWords):
			print("Incompatible function name at line=",l)
			sys.exit()
		if(functionID in functionIdList):
			print("Function with the same name already exists at line=",l)
			sys.exit()
		...
		if(procedureID in reservedWords):
			print("Incompatible procedure name at line=", l)
			sys.exit()
		if(procedureID in procedureIdList):
			print("Procedure with the ssame name already exist at line=",l)
			sys.exit()
		...
		if(token==")"):
			...
			if(token=="then"):
				...
				if(token=="}"):
					...
					print("If statement never close at line=",l)
					sys.exit()
		else:
			print(" ')' was expected at line=",l)
			sys.exit()
	...
	if(token=="("):
		...
		if(token==")"):
			...
			if(token=="}"):
				...
				print("While statement never stop at line=",l)
				sys.exit()
		else:
			print(" ')' was expected at line=",l)
			sys.exit()
	...
	if(token=="default"):
		...
		if(token==":"):
			...
			if(token=="}"):
				...
				print("For case never sto at line=",l)
				sys.exit()
		else:
			print(" ':' was expected at line=",l)
			sys.exit()
...
def returnStat():
	...
	if(internalFunction==False):
		print("Return statement is out of function at line=",l)
		sys.exit()
"""	
def callStat():
	if(token==idtk):
...
def inputStat():	
	global token
	token=lex()
	if(token not in reservedWords && token.isalnum()):
		genquad("inp", token, "_", "_")
		token=lex()
	else:
		print("Error with ID in input expression at line=",l)
		sys.exit()
...
def term():
	...
	if(factorTemp not in functionIdList):
		print("Function: ",factorTemp, "not declared at line=",l)
		sys.exit()
	while(token=="*" || token=="/"):
		tempToken=token
		token=lex()
...
def factor():
	...
	if(token.isdigit()):
		...
		if(token==")"):
			...
			print(" ')' was expected at line=",l)
			sys.exit()
	elif(token not in reservedWords && token.isalnum):
		tempConstant=token
		token=lex()
		...
		print("Problem with factor function at line=",l)
		sys.exit()
	return tempConstant
...
def addoper():
	...
	if(token=="+"):
		...
		print("invalid addoper at line=",l)
		sys.exit()
...
def muloper():
	...
	if(token=="*"):
		...
		print("invalid muloper at line=",l)
		sys.exit()
...
def genquad(op,x, y, z):
	...
	if(z in reservedWords):
		print("Error in name of z at line=",l)
		sys.exit()
...
def minQuadToCquad(quad, iterator):
	...
	if(quad[0]=="jump"):
	...
	elif(quad[0] in mulOpers || quad[0] in addOpers):
		...
		"""
	return cQuad="L_"+iterator+": "+cQuad
